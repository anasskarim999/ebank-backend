...
public class EbankBackendApplication {
    ...
    CommandLineRunner commandLineRunner(BankAccountService bankAccountService) {
        return args -> {
            Stream.of("Hassan","Imane","Anass").forEach(name->{
            ...
            bankAccountService.listCustomers().forEach(customer->{
                try {
                    ...
                    List<BankAccountDTO> banckAccounts= bankAccountService.banckAccountList();
                    for(BankAccountDTO banckAccount:banckAccounts){
                        for(int i=0 ;i<10 ; i++){
                            String accountId;
    ...
    CommandLineRunner start(CustomerRepository customerRepository, BankAccountRepository bankAccountRepository
        ...
        return args -> {
            Stream.of("Hassan","Yassin","Anass").forEach(name->{
            ...
            bankAccountRepository.findAll().forEach(acc->{
                for (int i=0 ;i<10 ;i++){
                    ...
                    accountOperation.setOperationDte(new Date());
